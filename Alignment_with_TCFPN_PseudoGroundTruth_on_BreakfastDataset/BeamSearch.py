...
class BeamSearch:
    ...
    def search(self,var_save_path,X_features,gt_actions,vid_info,vid_obj_info,VisModelTempPred,min_length_list,LengthNet,B,action2atomic_action,act2object_map,min_length_enable,action2step_map,action2bin_size):
        e=1e-20
        ...
        while(1):
                ...
                for i in range(len(video.sequences)):
                    ...
                    if length_sofar == 0:
                        ...
                        if length_sofar * 15 + LengthNet.duration > len(X_features[v]):
                            ...
                            xf.append(
                                np.concatenate((x, np.zeros([LengthNet.max_length - len(x), LengthNet.feature_size])), axis=0))
                        else:
            ...
            for v,video in enumerate(vid_list):
                    ...
                    for i in range(len(video.sequences)):
                        ...
                        acts_prob={}
                        t = e
                        for a in gt_actions[v]: #best done_vid: #1,1,40
                        ...
                        for a in range(video.index_tracker[i]):
                            ...
                            for j in range(len(length_dist[0,:])): #iterate over all length values
                                ...
                                if length_sofar_temp >= video.max_length:
                                    ...
                                    if overhead<=4:
                                        candidate = [seq + [proposed_length-overhead], score-1*log(length_dist[data_pointer,j])-1*log(act_prob+e), length_sofar_temp,actions+[action_candidates[a]]]
                                    else:
                                        break
                                else:
                                    candidate = [seq + [proposed_length], score-1*log(length_dist[data_pointer,j])-1*log(act_prob+e),length_sofar_temp,actions+[action_candidates[a]]]
