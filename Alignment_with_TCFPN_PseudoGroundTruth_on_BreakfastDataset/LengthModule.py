...
class LengthModule:
    ...
    def Soft_Label(self,x,y,length):
        ...
        mu=y
        soft_lb=np.ones([length])*Gaussian(x,mu,3)
    ...
    def generate_X_Y(self,data,predicted_actions,action2atomic_action,action2object_map,shuffle,gt,per_vid_acts,per_vid_obj_acts):
        ...
        def unison_shuffled_copies(a, b,c,d,e,f,g,h):
        ...
        for vid_num,vid in enumerate(predicted_actions):
            ...
            for n in range(length):
                ...
                indices.append(15 * self.sample_rate * n)  # the index in the original time scale (1/15 second scale)
                N.append(n_prev_seg)
        ...
        if shuffle:
            ...
            #X_actions=[#samples,]
            #X_features=[#samples,30,64]
            #Y=[#samples,length] in case of soft labeling
            #N=[#samples,]
    ...
    def train(self,First_Time,training_pack, val_pack,class_weights, keep_p,var_save_path, num_epochs, lr, regul):
            ...
            for n_ep, (data_per_eopch) in enumerate(epoch_gen(training_pack, self.batch_size, num_epochs)):
                ...
                if n_ep % 2 == 0:
                    ...
                    print("Test. || Epoch " + str(n_ep) + " :" + "Loss is :" + str(val_loss_values) + " and accuracy is: " + str(val_acc) +" and prox#1 is "+ str(np.average(proximity_val1))+" and prox#2 is "+ str(np.average(proximity_val2)))
                    ...
                    print("Base: Test. || Epoch " + str(n_ep) + " :"  + " and accuracy is: " + str(base_acc_val))
